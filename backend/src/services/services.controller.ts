...
class PatchProServiceDto {
  @IsOptional() @Type(() => Number) @IsInt() @Min(5) durationMin?: number;
...
class UpsertAddOnDto {
  @IsOptional() @IsUUID() id?: string;
  @IsString() @MinLength(1) name!: string;
  @IsOptional() @IsString() description?: string;
  @Type(() => Number) @IsInt() @Min(0) price!: number;
  @IsOptional() @Type(() => Number) @IsInt() @Min(0) extraDurationMin?: number;
  @IsOptional() @Type(() => Number) @IsInt() sortOrder?: number;
  @IsOptional() @IsBoolean() isActive?: boolean;
}
...
export class ServicesController {
  ...
  /** Full multi-level hierarchy (same payload as categories; explicit alias) */
  @Public()
  @Get('services/hierarchy')
  hierarchy() {
    return this.service.listHierarchy();
  }
  ...
  createCategory(
    ...
    @Body() dto: CreateCategoryNodeDto,
  ) {
    return this.service.createCategoryNode(userId, dto);
  }
  ...
  createService(
    @CurrentUser('id') userId: string,
    @Body() dto: CreateServiceNodeDto,
  ) {
    return this.service.createServiceNode(userId, dto);
  }
  ...
  @ApiBearerAuth()
  @Roles('professional', 'admin')
  @Patch('professionals/me/services/:id')
  patchProService(
    @CurrentUser('id') userId: string,
    @Param('id') id: string,
  ...
  @ApiBearerAuth()
  @Roles('professional', 'admin')
  @Get('professionals/me/services/:psId/add-ons')
  listAddOns(@CurrentUser('id') userId: string, @Param('psId') psId: string) {
    return this.service.listAddOns(userId, psId);
  }

  @ApiBearerAuth()
  @Roles('professional', 'admin')
  @Post('professionals/me/services/:psId/add-ons')
  upsertAddOn(
    @CurrentUser('id') userId: string,
    @Param('psId') psId: string,
    @Body() dto: UpsertAddOnDto,
  ) {
    return this.service.upsertAddOn(userId, psId, dto);
  }

  @ApiBearerAuth()
  @Roles('professional', 'admin')
  @Delete('professionals/me/add-ons/:id')
  deactivateAddOn(@CurrentUser('id') userId: string, @Param('id') id: string) {
    return this.service.deactivateAddOn(userId, id);
  }

  @ApiBearerAuth()
  @Roles('professional', 'admin')
  @Get('professionals/me/services/:psId/media')
  listMedia(@CurrentUser('id') userId: string, @Param('psId') psId: string) {
    return this.service.listMediaForProfessionalService(userId, psId);
  }

  @ApiBearerAuth()
  @Roles('professional', 'admin')
  @Post('professionals/me/services/:psId/media')
  attachMedia(
    @CurrentUser('id') userId: string,
    @Param('psId') psId: string,
    @Body() dto: AttachMediaDto,
  ) {
    return this.service.attachMediaToProfessionalService(userId, psId, dto.mediaId);
  }

  @ApiBearerAuth()
  @Roles('professional', 'admin')
  @Delete('professionals/me/services/:psId/media/:mediaId')
  detachMedia(
    @CurrentUser('id') userId: string,
    @Param('psId') psId: string,
    @Param('mediaId') mediaId: string,
  ) {
    return this.service.detachMediaFromProfessionalService(userId, psId, mediaId);
  }
