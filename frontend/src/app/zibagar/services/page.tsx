...
function findCategory(cats: CatalogCategory[], id: string): CatalogCategory | null {
  for (const c of cats) {
    if (c.id === id) return c;
    if (c.children?.length) { const f = findCategory(c.children, id); if (f) return f; }
  }
  return null;
}
...
function flattenSearch(cats: CatalogCategory[]) {
  ...
  function walk(c: CatalogCategory, path: string[], rootId: string) {
    ...
    for (const s of c.services || []) out.push({ type: 'svc', id: s.id, name: s.name, path: [...p, s.name].join(' › '), rootId });
...
function serviceLabel(ps: ProfessionalServiceItem) { return ps.service?.name?.trim() || 'خدمت'; }
