...
export default async function SearchPage({ searchParams }: Props) {
  ...
  try {
    result = await searchProfessionals({ q, city, category, page, limit: 12 });
  } catch (e) {
    errorMsg =
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      ...
      <p className="mt-1 text-sm text-gray">
        فیلتر بر اساس متن، شهر و دسته‌بندی (طبق API بک‌اند)
      </p>

      <form
        method="get"
        action="/search"
        className="mt-6 grid gap-3 rounded-3xl border border-border bg-white p-4 shadow-sm sm:grid-cols-2 lg:grid-cols-4"
      >
        <div>
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-gray">دسته</label>
          <select
            name="category"
            defaultValue={category || ''}
            className="h-11 w-full rounded-2xl border border-border bg-white px-3 text-sm outline-none focus:border-coral"
          >
        ...
        <div className="flex items-end">
          <button
            type="submit"
            className="h-11 w-full rounded-2xl bg-coral text-sm font-medium text-white hover:bg-[#e85a4c]"
          >
            اعمال فیلتر
          </button>
        </div>
      ...
      <p className="mt-3 text-xs text-gray">
        فیلتر قیمت و امتیاز به‌صورت query در API فعلی پشتیبانی نمی‌شود؛ مرتب‌سازی
        پیش‌فرض سرور: ویژه و امتیاز.
      </p>
