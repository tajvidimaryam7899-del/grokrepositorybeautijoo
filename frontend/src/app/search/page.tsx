...
export default async function SearchPage({ searchParams }: Props) {
  ...
  try { categories = await listFilterCategories(); } catch { /* filter categories are optional */ }
  try { result = await searchProfessionals({ q, city, category, page, limit: 12 }); }
  catch (e) { errorMsg = e instanceof PublicApiError ? e.message : 'خطا در دریافت نتایج جستجو'; }
  const totalPages = result ? Math.max(1, Math.ceil(result.meta.total / result.meta.limit)) : 1;
  function pageHref(p: number) {
    const params = new URLSearchParams();
    if (q) params.set('q', q); if (city) params.set('city', city); if (category) params.set('category', category); if (p > 1) params.set('page', String(p));
    const qs = params.toString(); return `/search${qs ? `?${qs}` : ''}`;
  }
  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      <h1 className="text-2xl font-bold">جستجو</h1>
      <p className="mt-1 text-sm text-gray">فیلتر بر اساس متن، شهر و دسته‌بندی‌های تأییدشده سایت</p>
      ...
      <div className="mt-8">
        ...
        {!errorMsg && result && result.items.length > 0 && <>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{result.items.map((pro) => <ProfessionalCard key={pro.id} pro={pro} />)}</div>
          {totalPages > 1 && <nav className="mt-8 flex flex-wrap items-center justify-center gap-2">
            {page > 1 && <Link href={pageHref(page - 1)} className="rounded-xl border border-border px-4 py-2 text-sm hover:bg-gray-light">قبلی</Link>}
            <span className="text-sm text-gray">صفحه {page.toLocaleString('fa-IR')} از {totalPages.toLocaleString('fa-IR')}</span>
            {page < totalPages && <Link href={pageHref(page + 1)} className="rounded-xl border border-border px-4 py-2 text-sm hover:bg-gray-light">بعدی</Link>}
          </nav>}
        </>}
      </div>
    </div>
