...
import { Prisma, ServiceCategoryAssignmentStatus } from '@prisma/client';
...
export class ServiceFiltersService {
  ...
  async listAllowedCategories(serviceId: string) {
    return this.prisma.$queryRaw<Array<{ id: string; name: string; slug: string; parentId: string | null; sortOrder: number }>>(Prisma.sql`
      ...
      JOIN service_categories c ON c.id = scs.category_id
  ...
  async listMyCategories(userId: string, professionalServiceId: string) {
    ...
    return this.prisma.$queryRaw<Array<{ id: string; name: string; slug: string; parentId: string | null; allowed: boolean; status: string | null }>>(Prisma.sql`
      SELECT c.id, c.name, c.slug, c.parent_id AS "parentId", true AS allowed,
             psc.status::text AS status
      FROM service_category_services scs
      JOIN service_categories c ON c.id = scs.category_id
      LEFT JOIN professional_service_categories psc
        ON psc.category_id = c.id AND psc.professional_service_id = ${professionalServiceId}::uuid
  ...
  async requestCategory(userId: string, professionalServiceId: string, categoryId: string) {
    ...
    const own = await this.prisma.professionalService.findFirst({ where: { id: professionalServiceId, professionalId: pro.id }, select: { serviceId: true } });
    if (!own) throw new NotFoundException('خدمت زیباگر یافت نشد');
    const allowed = await this.prisma.$queryRaw<Array<{ ok: boolean }>>(Prisma.sql`
      SELECT EXISTS(
        SELECT 1 FROM service_category_services
        WHERE service_id = ${own.serviceId}::uuid AND category_id = ${categoryId}::uuid AND is_active = true
      ) AS ok
    `);
    if (!allowed[0]?.ok) throw new BadRequestException('این دسته‌بندی برای این تخصص مجاز نیست');
    await this.prisma.$executeRaw(Prisma.sql`
      INSERT INTO professional_service_categories (professional_service_id, category_id, status)
      VALUES (${professionalServiceId}::uuid, ${categoryId}::uuid, 'pending')
      ON CONFLICT (professional_service_id, category_id)
      DO UPDATE SET status = CASE WHEN professional_service_categories.status = 'rejected' THEN 'pending' ELSE professional_service_categories.status END,
  ...
  async removeMyCategory(userId: string, professionalServiceId: string, categoryId: string) {
    ...
    if (!own) throw new NotFoundException('خدمت زیباگر یافت نشد');
    await this.prisma.$executeRaw(Prisma.sql`
      DELETE FROM professional_service_categories
      WHERE professional_service_id = ${professionalServiceId}::uuid AND category_id = ${categoryId}::uuid AND status <> 'rejected'
    `);
    return this.listMyCategories(userId, professionalServiceId);
  }
  ...
  async createCategory(data: { name: string; parentId?: string | null; slug?: string; description?: string; icon?: string; sortOrder?: number; isActive?: boolean }) {
    ...
    if (data.parentId) {
      const parent = await this.prisma.serviceCategory.findUnique({ where: { id: data.parentId } });
      if (!parent) throw new NotFoundException('دسته والد یافت نشد');
    }
    return this.prisma.serviceCategory.create({
      data: { name, slug, parentId: data.parentId ?? null, description: data.description?.trim() || null, icon: data.icon?.trim() || null, sortOrder: data.sortOrder ?? 0, isActive: data.isActive ?? true },
    });
  }
  ...
  async updateCategory(id: string, data: { name?: string; parentId?: string | null; slug?: string; description?: string; icon?: string; sortOrder?: number; isActive?: boolean }) {
    ...
    if (!existing) throw new NotFoundException('دسته یافت نشد');
  ...
  async assignCategoryToService(serviceId: string, categoryId: string, sortOrder = 0) {
    const [service, category] = await Promise.all([
      ...
      this.prisma.serviceCategory.findUnique({ where: { id: categoryId } }),
    ]);
    if (!service) throw new NotFoundException('تخصص یافت نشد');
    if (!category) throw new NotFoundException('دسته یافت نشد');
    await this.prisma.$executeRaw(Prisma.sql`
      INSERT INTO service_category_services (service_id, category_id, sort_order, is_active)
      VALUES (${serviceId}::uuid, ${categoryId}::uuid, ${sortOrder}, true)
      ON CONFLICT (service_id, category_id) DO UPDATE SET sort_order = EXCLUDED.sort_order, is_active = true, updated_at = CURRENT_TIMESTAMP
    `);
  ...
  async unassignCategoryFromService(serviceId: string, categoryId: string) {
    await this.prisma.$executeRaw(Prisma.sql`
      UPDATE service_category_services SET is_active = false, updated_at = CURRENT_TIMESTAMP
      WHERE service_id = ${serviceId}::uuid AND category_id = ${categoryId}::uuid
    `);
    return this.listAllowedCategories(serviceId);
  }

  async listAdminCategories() {
    return this.prisma.$queryRaw<Array<{ id: string; name: string; slug: string; parentId: string | null; isActive: boolean; sortOrder: number }>>(Prisma.sql`
      SELECT id, name, slug, parent_id AS "parentId", is_active AS "isActive", sort_order AS "sortOrder"
      FROM service_categories ORDER BY parent_id NULLS FIRST, sort_order ASC, name ASC
    `);
  }

  async listRequests(status?: 'pending' | 'approved' | 'rejected') {
    const rows = await this.prisma.$queryRaw<Array<Record<string, unknown>>>(Prisma.sql`
      SELECT psc.professional_service_id AS "professionalServiceId", psc.category_id AS "categoryId",
             psc.status::text AS status, psc.created_at AS "createdAt", psc.reviewed_at AS "reviewedAt",
             c.name AS "categoryName", c.slug AS "categorySlug",
             s.id AS "serviceId", s.name AS "serviceName",
             p.id AS "professionalId", p.title AS "professionalTitle"
      FROM professional_service_categories psc
      JOIN service_categories c ON c.id = psc.category_id
      JOIN professional_services ps ON ps.id = psc.professional_service_id
  ...
  async reviewRequest(adminUserId: string, professionalServiceId: string, categoryId: string, status: 'approved' | 'rejected' | 'pending') {
    ...
    const result = await this.prisma.$executeRaw(Prisma.sql`
      UPDATE professional_service_categories
      SET status = ${status}::"ServiceCategoryAssignmentStatus",
          reviewed_at = CASE WHEN ${status}::text = 'pending' THEN NULL ELSE CURRENT_TIMESTAMP END,
          reviewed_by = CASE WHEN ${status}::text = 'pending' THEN NULL ELSE ${adminUserId}::uuid END,
          updated_at = CURRENT_TIMESTAMP
      WHERE professional_service_id = ${professionalServiceId}::uuid AND category_id = ${categoryId}::uuid
    `);
    ...
    const [items, total] = await Promise.all([
      this.prisma.professional.findMany({
        ...
        include: {
          ...
          professionalServices: { where: { isActive: true }, take: 5, include: { service: { select: { name: true, slug: true } } } },
        },
      }),
