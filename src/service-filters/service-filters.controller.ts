...
export class ServiceFiltersController {
  ...
  publicCategories(@Param('serviceId') serviceId: string) {
    return this.service.listAllowedCategories(serviceId);
  }
  ...
  myCategories(@CurrentUser('id') userId: string, @Param('psId') psId: string) {
    return this.service.listMyCategories(userId, psId);
  }

  @ApiBearerAuth()
  @Roles('professional', 'admin')
  @Post('professionals/me/services/:psId/filter-categories')
  requestCategory(@CurrentUser('id') userId: string, @Param('psId') psId: string, @Body() dto: ProCategoryDto) {
    return this.service.requestCategory(userId, psId, dto.categoryId);
  }

  @ApiBearerAuth()
  @Roles('professional', 'admin')
  @Delete('professionals/me/services/:psId/filter-categories/:categoryId')
  removeMyCategory(@CurrentUser('id') userId: string, @Param('psId') psId: string, @Param('categoryId') categoryId: string) {
    return this.service.removeMyCategory(userId, psId, categoryId);
  }
  ...
  assignCategory(@Param('serviceId') serviceId: string, @Body() dto: AssignCategoryDto) {
    return this.service.assignCategoryToService(serviceId, dto.categoryId, dto.sortOrder);
  }

  @ApiBearerAuth()
  @Roles('admin')
  @Delete('admin/services/:serviceId/filter-categories/:categoryId')
  unassignCategory(@Param('serviceId') serviceId: string, @Param('categoryId') categoryId: string) {
    return this.service.unassignCategoryFromService(serviceId, categoryId);
  }

  @ApiBearerAuth()
  @Roles('admin')
  @Get('admin/service-category-requests')
  requests(@Query('status') status?: 'pending' | 'approved' | 'rejected') {
